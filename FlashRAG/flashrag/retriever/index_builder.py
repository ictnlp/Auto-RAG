...
class Index_Builder:
    ...
    def encode_all(self):
        ...
        all_embeddings = np.zeros((len(self.corpus), 768), dtype=np.float32)
        ...
        for start_idx in tqdm(range(0, len(self.corpus), self.batch_size), desc='Inference Embeddings:'):
            ...
            all_embeddings[start_idx:start_idx+self.batch_size] = embeddings

        print("Finish encoding all documents.")
