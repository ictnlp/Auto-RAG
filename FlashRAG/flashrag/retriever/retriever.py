...
class DenseRetriever(BaseRetriever):
    ...
    def __init__(self, config: dict):
        ...
        if True:
            # 指定使用哪些 GPU，例如，使用 GPU 0 和 GPU 1
            # 获取CUDA设备数量
            ...
            # 创建 GpuMultipleClonerOptions 对象
            co = faiss.GpuMultipleClonerOptions()
            co.useFloat16 = True
            co.shard = True

            # 将 CPU 索引转换为 GPU 索引，并指定 GPU
            self.index = faiss.index_cpu_to_gpus_list(self.index, co, gpu_ids, ngpu=2)
        self.corpus = load_corpus(self.corpus_path)
