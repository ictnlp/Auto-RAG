...
class IterativePipeline(BasicPipeline):
    ...
    def run(self, dataset, do_eval=True, pred_process_fun=None):
        ...
        for iter_idx in range(self.iter_num):
            ...
            retrieval_results = [self.retriever.batch_search(q)[0] for q in input_query]
            ...
            past_generation_result = [ self.generator.generate(p)[0] for p in input_prompts ]
...
class SelfRAGPipeline(BasicPipeline):
    ...
    def run_batch_pred(self, dataset):
        ...
        print(self.retriever.retrieve_times)
        return dataset
    ...
    def run_item(self, item):
        ...
        while gen_length < self.max_generation_length and iter_round < self.max_iter_num:
            ...
            round_gen_output, scores = self.generator.generate(
                input_prompt, return_scores=True, stop=self.stop_sym, max_tokens=self.look_ahead_steps)
...
class SelfAskPipeline(BasicPipeline):
    ...
    def __init__(self, config, prompt_template=None, max_iter=5, single_hop=False):
        super().__init__(config, prompt_template)
